BEGIN
    ...
    dbms_scheduler.create_job
        ( job_name             =>   'ANALYZETHIS.GATHER_STATS',
          ...
          job_action           =>   'analyzethis.GatherSchemaStats( p_schema => ''MySchema'', p_backup => TRUE, p_option => ''AUTO'');',
          start_date           =>   sysdate,
          repeat_interval      =>   'FREQ=DAILY;INTERVAL=1;BYHOUR=6;BYMINUTE=0;BYSECOND=0',
          enabled              =>   TRUE,
          auto_drop            =>   FALSE,
          comments             =>   'This job will gather statistics on the MySchema schema using the analyzethis package'
        );
