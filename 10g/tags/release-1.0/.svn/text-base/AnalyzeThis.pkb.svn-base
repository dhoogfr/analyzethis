...
-----------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        <SUMMARY>
            ...
            p_gather_sys = list sys objects, yes or no (only with p_schema = *)
        </SUMMARY>
        <MODIFICATIONS>
            <MODIFICATION>
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        <MODIFICATIONS>
            ...
            <MODIFICATION>
                ...
                <AUTHOR> Freek D'Hooge </AUTHOR>
                <WHAT> completely reworked the rules for determining which options will be retrieved from the statoptions table
                       by the GetSubpartOpt, GetPartOpt en de GetOpt procedures.
                </WHAT>
            </MODIFICATION>
        </MODIFICATIONS>
*/
...
-- START OF INLINE MODULES
        ...
        BEGIN
            ...
            l_StatOptions.auto_sample_size  :=  'TRUE';
            l_StatOptions.estimate_percent  :=  NULL;
            l_StatOptions.block_sample      :=  'FALSE';
            l_StatOptions.method_opt        :=  'FOR ALL COLUMNS SIZE AUTO';
            l_StatOptions.default_degree    :=  'AUTO';
            l_StatOptions.degree            :=  NULL;
            l_StatOptions.granularity       :=  'AUTO';
            l_StatOptions.cascade           :=  'AUTO';
            l_StatOptions.no_invalidate     :=  'AUTO';
...
*/
    ...
    BEGIN
        ...
        debug.f('Open the l_obj_cursor ref cursor');
        -- don't use * in the query below as oracle will throw an ORA-22905: cannot access rows from a non-nested table item
        open l_obj_cursor for
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        <MODIFICATIONS>
            ...
            <MODIFICATION>
                ...
                <AUTHOR> Freek D'Hooge </AUTHOR>
                <WHAT>  added the force parameter to the dbms_stats.gather_index_stats
                        call in the calcstats subprocedure.   
                </WHAT>
            </MODIFICATION>
            <MODIFICATION>
                <DATE> 21/12/2007 </DATE>
            ...
            then
                ...
                dbms_stats.gather_table_stats
                    ( ownname           =>  '"' || p_ownname || '"',
                      ...
                      degree            =>  case when l_statoptions.default_degree = 'AUTO'
                                                   then dbms_stats.AUTO_DEGREE
                                                 when l_statoptions.default_degree = 'TRUE'
                                                   then dbms_stats.default_degree
                                                 else l_statoptions.degree
                                            end,
                      granularity       =>  l_statoptions.granularity,
                      cascade           =>  case when l_statoptions.cascade = 'AUTO'
                                                   then dbms_stats.AUTO_CASCADE
                                                 when l_statoptions.cascade = 'TRUE'
                                                   then TRUE
                                                 else FALSE
                                            end,
                      no_invalidate     =>  case when l_statoptions.no_invalidate = 'AUTO'
                                                   then dbms_stats.AUTO_INVALIDATE
                                                 when l_statoptions.no_invalidate = 'TRUE'
                                                   then TRUE
                                                 else FALSE
                                            end,
                    force               =>  true
